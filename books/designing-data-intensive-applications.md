...
UTKARSH
https://www.youtube.com/watch?v=JZGtV278SvE
...
utkarsh
https://www.youtube.com/watch?v=VSX4a3h4SmQ
...
Many databases use a _log_ (more precisely a write-ahead log which has 1 writer thread and the logs are broken into segments which can finally be flushed and reduced to single segment) which is append-only data file. Real databases have more issues to deal with tho (concurrency control, reclaiming disk space so the log doesn't grow forever and handling errors and partially written records).
